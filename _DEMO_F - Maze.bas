' DEMO F: In this demo, we turn our forest into a maze. We add a function to quickly draw the map
' PUT tile drawing has been replaced entirely with drawTile and drawTileToPlayfield
' The tree tile still draws using LOCATE/PRINT.

CONST SCREENSIZEX = 288 ' A little lower resolution now
...
SUB createtiles ()
  ...
  ' Tile 0, tNumBlank simply remains clear
...
SUB redrawAll ()
  ...
  FOR iy = 0 TO (viewSizeTilesY - 1)
    FOR ix = 0 TO (viewSizeTilesX - 1)
      ...
      IF map(tempX, tempY) = mObjWall THEN ' Draw characters like trees and walls first
        ...
        ZLOCATE ix + (viewStartPixelsX \ 8), iy + (viewStartPixelsY \ 8)
        COLOR 15
        PRINT CHR$(tNumWall); ' Our walls are just an ASCII character, like the trees
  ...
  ' Draw heart:
  drawTile textX * 8, 8 * 1, tNumHeart ' draw heart, if INFRAME had been used, it wouldn't go outside the viewing area
...
SUB ESCAPE (code)
  COLOR 15
  ZLOCATE 10, 5: PRINT "                              "
  ZLOCATE 10, 6: PRINT "PROGRAM ENDED WITH CODE:"; code; "    "
  ZLOCATE 10, 7: PRINT "                             "
